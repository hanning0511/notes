...
If you have to write strings that are longer than 120 characters, this should be done with a here document or an embedded newline if possible. Literal strings that have to be longer than 120 chars and can’t sensibly be split are ok, but it’s strongly preferred to find a way to make it shorter.
...
`<` and `>` don’t perform numerical comparison inside `[[ … ]]` expressions (they perform lexicographical comparisons instead; see Testing Strings. For preference, don’t use `[[ … ]]` _at all_ for numeric comparisons, use `(( … ))` instead.
